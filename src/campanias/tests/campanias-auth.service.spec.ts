...
describe('CampaniasAuthService', () => {
  it('resuelve usuario por auth_id', async () => {
    const supabase = createSupabase({
      data: { id: 1, nombre: 'Admin', rol: 'ADMIN' },
      error: null,
    });
    const service = new CampaniasAuthService(supabase);
  ...
  describe('assertAdmin', () => {
    const service = new CampaniasAuthService({} as any);
