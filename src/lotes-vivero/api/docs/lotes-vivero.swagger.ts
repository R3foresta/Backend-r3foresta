...
export function ApiCrearEvidenciaPendiente() {
  return applyDecorators(
    ApiOperation({
      summary: 'Crear evidencia pendiente de vivero',
      description:
        'Sube fotos como evidencias pendientes de asociar a un lote de vivero. Admite hasta 5 fotos por solicitud.',
    }),
    ApiHeader(AUTH_ID_HEADER),
    ApiConsumes('multipart/form-data'),
    ApiBody({
      description: 'Datos de la evidencia con fotos adjuntas (multipart/form-data)',
      schema: {
        type: 'object',
        properties: {
          titulo: {
            type: 'string',
            maxLength: 120,
            example: 'Inicio de germinacion',
          },
          descripcion: {
            type: 'string',
            maxLength: 1000,
            example: 'Fotos del primer dia en cama de germinacion',
          },
          metadata: {
            type: 'string',
            example: '{"camara": "iPhone 14", "condicion": "sombra"}',
    ...
    ApiResponse({ status: 201, description: 'Evidencia pendiente creada exitosamente' }),
    ApiResponse({ status: 400, description: 'Datos invalidos o formato de archivo no permitido' }),
...
export function ApiCrearLoteDesdeRecoleccion() {
  return applyDecorators(
    ApiOperation({
      summary: 'Crear lote de vivero desde recoleccion',
      description:
        'Inicia un lote de vivero usando el saldo materializado de una recoleccion validada. La operacion es atomica: crea el lote, el evento de inicio y descuenta el saldo de la recoleccion.',
    }),
    ApiHeader(AUTH_ID_HEADER),
    ApiBody({
      schema: {
        ...
        properties: {
          ...
          evidencia_ids: {
            ...
            example: [501, 502],
            description: 'IDs de evidencias pendientes a asociar al evento de inicio',
          },
          observaciones: {
            type: 'string',
            maxLength: 1000,
            example: 'Primera siembra del ciclo primavera 2026',
          },
        },
      },
    }),
    ApiResponse({ status: 201, description: 'Lote de vivero creado exitosamente' }),
    ApiResponse({ status: 400, description: 'Datos invalidos o saldo de recoleccion insuficiente' }),
    ApiResponse({ status: 401, description: 'Header x-auth-id requerido' }),
    ApiResponse({ status: 404, description: 'Recoleccion o vivero no encontrado' }),
    ApiResponse({ status: 500, description: 'Error interno del servidor' }),
  );
}
...
export function ApiRegistrarEmbolsado() {
  return applyDecorators(
    ...
    ApiResponse({ status: 201, description: 'Embolsado registrado exitosamente' }),
    ApiResponse({ status: 400, description: 'Datos invalidos o lote en estado incorrecto' }),
    ApiResponse({ status: 401, description: 'Header x-auth-id requerido' }),
...
export function ApiRegistrarAdaptabilidad() {
  return applyDecorators(
    ...
    ApiResponse({ status: 201, description: 'Adaptabilidad registrada exitosamente' }),
    ApiResponse({ status: 400, description: 'Datos invalidos o transicion de subetapa no permitida' }),
    ApiResponse({ status: 401, description: 'Header x-auth-id requerido' }),
    ApiResponse({ status: 404, description: 'Lote de vivero no encontrado' }),
    ApiResponse({ status: 500, description: 'Error interno del servidor' }),
  );
...
export function ApiRegistrarMerma() {
  return applyDecorators(
    ...
    ApiResponse({ status: 400, description: 'Datos invalidos o cantidad supera el stock vivo actual' }),
    ApiResponse({ status: 401, description: 'Header x-auth-id requerido' }),
    ApiResponse({ status: 404, description: 'Lote de vivero no encontrado' }),
    ApiResponse({ status: 500, description: 'Error interno del servidor' }),
  );
...
export function ApiRegistrarDespacho() {
  return applyDecorators(
    ...
    ApiBody({
      schema: {
        type: 'object',
        required: ['fecha_evento', 'cantidad_afectada', 'destino_tipo', 'destino_referencia'],
        properties: {
          fecha_evento: {
            type: 'string',
            format: 'date',
          ...
          comunidad_destino_id: {
            ...
            minimum: 1,
            example: 3,
            description: 'ID de la comunidad destino (requerido si destino_tipo es DONACION_COMUNIDAD)',
          },
          observaciones: {
            type: 'string',
            maxLength: 1000,
    ...
    ApiResponse({ status: 201, description: 'Despacho registrado exitosamente' }),
    ApiResponse({
      status: 400,
...
export function ApiListarLotes() {
  return applyDecorators(
    ...
    ApiQuery({ name: 'vivero_id', required: false, type: Number, description: 'ID del vivero' }),
    ApiQuery({
      name: 'recoleccion_id',
      required: false,
      type: Number,
    ...
    ApiResponse({ status: 200, description: 'Lista de lotes de vivero obtenida' }),
...
export function ApiObtenerTimeline() {
  return applyDecorators(
    ...
    ApiResponse({ status: 200, description: 'Timeline del lote obtenido exitosamente' }),
    ApiResponse({ status: 400, description: 'Parametros de filtro invalidos' }),
    ApiResponse({ status: 404, description: 'Lote de vivero no encontrado' }),
    ApiResponse({ status: 500, description: 'Error interno del servidor' }),
  );
